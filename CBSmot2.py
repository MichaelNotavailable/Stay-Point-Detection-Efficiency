...
class CBSmot2:
    ...
    def count_neighbors(self, traj, position, max_dist):
        ...
        while j < len(traj.index) and yet:
            if self.calculate_two_point_distance(traj.iloc[position]['lat'],
                                                        traj.iloc[position]['lng'],
                                                        traj.iloc[j]['lat'],
                                                        traj.iloc[j]['lng']) < max_dist:
                neighbors += 1
            else:
                yet = False
            j += 1
        ...
        while i < len(stops):
            if (i+1) < len(stops):
                ...
                if (p2 - p1).item() / 1000000000 <= time_tolerance:
                    c1 = self.centroid(s1)
                    c2 = self.centroid(s2)
                    if self.calculate_two_point_distance(c1[0], c1[1], c2[0], c2[1]) <= max_dist:
                        stops.pop(i+1)
    ...
    def calculate_two_point_distance(self, lat, lon, lat2, lon2):
        r = 6372.8
